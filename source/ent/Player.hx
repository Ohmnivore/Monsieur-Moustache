...
class DeathEmitter extends FlxEmitter
{
	...
	public function new(P:Player)
	{
		...
		loadParticles("images/particle.png", 5, 0, false, false);
		
		//setAlpha(0.8, 1.0, 0.4, 0.6);
		alpha.set(0.6, 0.7, 0.1, 0.1);
		//setColor(Player.ARM_COLOR, 0xff6AD622);
		color.set(Player.ARM_COLOR, Player.ARM_COLOR, 0xff6AD622, 0xff6AD622);
		//setYSpeed( -5, 1);
		velocity.set(-20, -5, 20, 1, -20, -5, 20, 1);
		//setXSpeed( -20, 20);
...
class StretchSprite extends FlxSprite
{
	...
	private function drawFists():Void
	{
		FlxSpriteUtil.drawRect(this, _end.x - 6 - 1, _end.y - 1,
			...
			Player.FIST_COLOR, FISTLINESTYLE, DRAWSTYLE);
...
class JumpDust extends FlxEmitter
{
	public function new(X:Float, Y:Float)
	{
		...
		loadParticles("images/particleSmall.png", 10, 0, false, false);
		
		//setColor(0xffC4C4C4, 0xffA3A3A3);
		color.set(0xffC4C4C4, 0xffC4C4C4, 0xffA3A3A3, 0xffA3A3A3);
		//setAlpha(0.5, 0.75, 0.0, 0.0);
		alpha.set(0.5, 0.75, 0.0, 0.0);
		//setXSpeed(-30, 30);
		//setYSpeed(-12, 0);
		velocity.set(-30, -12, 30, 0, -30, -12, 30, 0);
		
		//setRotation(0, 0);
		angularVelocity.set(0, 0, 0, 0);
		//
		//start(true, 0.5, 0.1, 0, 0.5);
		elasticity.set(0.5, 0.5, 0.5, 0.5);
		allowCollisions = FlxObject.DOWN;
		start(true, 0.5, 0);
	}
}