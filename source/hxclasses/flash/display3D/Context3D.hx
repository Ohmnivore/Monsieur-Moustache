...
@:final extern class Context3D extends flash.events.EventDispatcher {
	...
	function clear(red : Float = 0, green : Float = 0, blue : Float = 0, alpha : Float = 0, depth : Float = 0, stencil : UInt = 0, mask : UInt = 0) : Void;
	function configureBackBuffer(width : Int, height : Int, antiAlias : Int, enableDepthAndStencil : Bool = false, wantsBestResolution : Bool = false) : Void;
	...
	function dispose(recreate : Bool = false) : Void;
	...
	function setProgramConstantsFromVector(programType : Context3DProgramType, firstRegister : Int, data : flash.Vector<Float>, numRegisters : Int = 0) : Void;
	...
	function setStencilReferenceValue(referenceValue : UInt, readMask : UInt = 0, writeMask : UInt = 0) : Void;
