...
@:final extern class Graphics {
	function new() : Void;
	function beginBitmapFill(bitmap : BitmapData, ?matrix : flash.geom.Matrix, repeat : Bool = false, smooth : Bool = false) : Void;
	function beginFill(color : UInt, alpha : Float = 0) : Void;
	...
	@:require(flash10) function lineBitmapStyle(bitmap : BitmapData, ?matrix : flash.geom.Matrix, repeat : Bool = false, smooth : Bool = false) : Void;
	...
	function lineStyle(thickness : Null<Float> = 0, color : UInt = 0, alpha : Float = 0, pixelHinting : Bool = false, ?scaleMode : LineScaleMode, ?caps : CapsStyle, ?joints : JointStyle, miterLimit : Float = 0) : Void;
