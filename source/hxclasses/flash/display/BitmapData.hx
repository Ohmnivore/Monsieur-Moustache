...
extern class BitmapData implements IBitmapDrawable {
	...
	function getColorBoundsRect(mask : UInt, color : UInt, findColor : Bool = false) : flash.geom.Rectangle;
	...
	function hitTest(firstPoint : flash.geom.Point, firstAlphaThreshold : UInt, secondObject : flash.utils.Object, ?secondBitmapDataPoint : flash.geom.Point, secondAlphaThreshold : UInt = 0) : Bool;
	function lock() : Void;
	...
	function noise(randomSeed : Int, low : UInt = 0, high : UInt = 0, channelOptions : UInt = 0, grayScale : Bool = false) : Void;
	...
	function perlinNoise(baseX : Float, baseY : Float, numOctaves : UInt, randomSeed : Int, stitch : Bool, fractalNoise : Bool, channelOptions : UInt = 0, grayScale : Bool = false, ?offsets : Array<flash.geom.Point>) : Void;
	...
	function threshold(sourceBitmapData : BitmapData, sourceRect : flash.geom.Rectangle, destPoint : flash.geom.Point, operation : String, threshold : UInt, color : UInt = 0, mask : UInt = 0, copySource : Bool = false) : UInt;
